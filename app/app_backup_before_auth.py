...
# ============================================================
# PHASE 3 — STEP 3: AUTHENTICATION & USER ROLES
# ============================================================
...
def hash_password(password):
    """Create a SHA-256 password hash for local application login."""
    return hashlib.sha256(password.encode("utf-8")).hexdigest()


def ensure_default_admin():
    """Initialize the database and create the first admin if needed."""
    initialize_database()
...
def authenticate_user(username, password):
    ...
    # New accounts use SHA-256 hashes. The second comparison keeps
    # compatibility with any older local account that stored plaintext.
...
def render_login_page():
    """Render the login screen and stop the rest of the app until login."""
    st.markdown(
    ...
    st.markdown(
        '<div class="subtitle">Secure local access to financial research tools.</div>',
        unsafe_allow_html=True,
    )
    ...
    with center:
        ...
        st.info(
            "Initial administrator: username `admin`, password `admin123`. "
            "Change the administrator password before production deployment."
        )
...
# Initialize the database and first administrator before loading the
# potentially expensive embedding model.
    ...
    try:
        history = get_research_history(
            ...
            user_id=(
                ...
                else st.session_state.get("current_user_id")
            ),
        )
...
with st.sidebar:
    ...
    st.success(
        f"👤 {st.session_state.get('current_username', 'User')} "
        f"({str(st.session_state.get('current_role', 'user')).title()})"
    )
